...
import { DEMO_FIXED_LOCATION, USE_FIXED_LOCATION } from '../../config/mapDemoTuning';
...
type LocationSource = 'device' | 'mock' | 'fixed';

let currentCoordinate: { coordinate: TaggedCoordinate; source: LocationSource } | undefined;

async function resolveCoordinate(fallback: LocationProvider): Promise<{ coordinate: TaggedCoordinate; source: LocationSource }> {
  // demo 阶段不触发浏览器/高德定位；设回 false 后下方真实定位分支保持原样恢复。
  if (USE_FIXED_LOCATION) {
    return {
      coordinate: { latitude: DEMO_FIXED_LOCATION.lat, longitude: DEMO_FIXED_LOCATION.lng, system: INTERNAL_SYSTEM },
      source: 'fixed',
    };
  }
  try {
...
async function locatePlaceFromCoordinate(coordinate: TaggedCoordinate, source: LocationSource, seed: string): Promise<LocatedPlace> {
  const { place, placeSource } = await resolvePlace(coordinate);
